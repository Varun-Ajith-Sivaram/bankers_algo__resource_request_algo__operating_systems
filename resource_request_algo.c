...
int* get_details(int x,int y,int[x],int[x][y],int[x][y],int[y]);
...
void resource_request(int x,int y,int[x],int[x][y],int[x][y],int[y]);

int i,j,k,l,m,total,res_total;

void main()
{
    printf("\tImplementation of Resource Request Algorithm\n\n");
    ...
    int *initial_avail;
    initial_avail = get_details(total,res_total,Process,Allocated,Max_resources,Available);
    ...
    for(i = 0; i < res_total; i++)
    {
        Available[i] = initial_avail[i];
    }
    
    printf("\n\n Enter the details of new request:\n");
    resource_request(total,res_total,Process,Allocated,Need_resources,Available);
...
int* get_details(int x,int y,int p[x],int alloc[x][y],int max[x][y],int avail[y])
{
    int *a = malloc(sizeof(y));
...
void resource_request(int x,int y,int p[x],int alloc[x][y],int need[x][y],int avail[y])
{
    int index,pid,req[y],flag1 = 0,flag = 0,condition = 1;
    ...
    while(condition)
    {
        ...
        {
            condition = 1;
            printf("\n\t   --> Invalid process id, Enter a valid process id!!\n");
        }
    ...
    printf("\n\t   Enter details of requested resources of Process %d:\n",p[index]);
        
    for(j = 0; j < y; j++)
    {
        printf("\n\t\tRequested instances of Resource %d: ",j+1);
        scanf("%d",&req[j]);
    }
    printf("\n");
    ...
    for(j = 0; j < y; j++)
    {
        if(req[j] <= need[index][j] && req[j] <= avail[j])
        {
            flag = 1;
        }
        ...
        {
            printf("\n\n\t   --> Safe Sequence cannot be obtained!! Deadlock will occur!!\n");
            printf("\n\t   --> Request cannot be granted!!\n");
            exit(0);
        }
    }
